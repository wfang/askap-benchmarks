...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0) {
        std::cout << "+++++ Forward processing (MPI_OMP) +++++" << std::endl;
    }
    ...
    if (rank == 0) {
        std::cout << "    Number of processes: " << numtasks <<", each  with number of threads: "<<numthreads << std::endl;
        ...
        std::cout << "+++++ Reverse processing (MPI_OMP) +++++" << std::endl;
    }
